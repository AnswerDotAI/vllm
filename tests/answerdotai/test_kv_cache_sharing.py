...
def test_throughput(
    ...
    with vllm_runner(
        ...
        base_prefill_tput, base_decode_tput = 0.0, 0.0
        for _ in range(num_repeats):
            prefill_tput, decode_tput = _benchmark_speed(vllm_model, example_prompts)
            base_prefill_tput += prefill_tput / num_repeats
            base_decode_tput += decode_tput / num_repeats
    ...
    with vllm_runner(
        ...
        test_prefill_tput, test_decode_tput = 0.0, 0.0
        for _ in range(num_repeats):
            prefill_tput, decode_tput = _benchmark_speed(vllm_model, example_prompts)
            test_prefill_tput += prefill_tput / num_repeats
            test_decode_tput += decode_tput / num_repeats
    
    prefill_degradation = ((base_prefill_tput - test_prefill_tput) / base_prefill_tput) * 100
    decode_degradation = ((base_decode_tput - test_decode_tput) / base_decode_tput) * 100
    
    error_message = []
    if test_prefill_tput <= base_prefill_tput:
        error_message.append(f"Prefill throughput is degraded by {prefill_degradation:.2f}%: {test_prefill_tput:.2f} vs {base_prefill_tput:.2f}")
        
    if test_decode_tput <= base_decode_tput:
        error_message.append(f"Decode throughput is degraded by {decode_degradation:.2f}%: {test_decode_tput:.2f} vs {base_decode_tput:.2f}")
...
def test_q_proj_without_kv(
    ...
    """
    Test `q_proj` vs `qkv_proj` layer for decoder layers that share KV cache with previous layers.
    
    Decoder layers that share KV cache with previous layers have `q_proj` instead of `qkv_proj`.
    
    Weights of `model.layers.0.self_attn.q_proj.weight` and `model.layers.1.self_attn.q_proj.weight` 
    are the same in the test llama model checkpoint for testing purposes.
    """
    override_backend_env_variable(monkeypatch, backend)
    ...
    with vllm_runner(
        ...
        # Tests that weights are loaded correctly.
        qkv_layer = model.model.layers[0].self_attn.qkv_proj
        q_layer   = model.model.layers[1].self_attn.q_proj
        q_size = model.model.layers[0].self_attn.q_size
        assert torch.equal(qkv_layer.weight[:q_size], q_layer.weight)
        
        # Tests that output of `q_proj` matches `qkv_proj` for the first `q_size` elements.
        x = torch.randn((2, q_layer.input_size), dtype=q_layer.weight.dtype, device=q_layer.weight.device)
        assert torch.equal(q_layer(x)[0], qkv_layer(x)[0][:,:q_size])